...
pub fn handle_fight(lentity : &mut Entity, rentity : &mut Entity) -> String {
    ...
    if init_gen <  linitiative {
        fight_result_message.push_str(format!("{} tries to hit {}.\n", lentity.name, rentity.name).as_str());
        // Hit or miss
        if thread_rng().gen::<f32>() < get_hit_chance(lentity.get_effective_attack(), rentity.get_effective_defense()) {
            let dmg = get_damage(lentity.attack, rentity.defense);
            fight_result_message.push_str(format!("They hit for {} damage!\n", dmg as u16).as_str());
        ...
        if rentity.alive {
            fight_result_message.push_str(format!("{} attempts to strike back!\n", rentity.name).as_str());
            if thread_rng().gen::<f32>() < get_hit_chance(rentity.get_effective_attack(), lentity.get_effective_defense()) {
    ...
    if lentity.alive {
        ...
        if amount > 0 {
            fight_result_message.push_str(format!("{} regenerated {} health.\n", lentity.name, amount).as_str());
        }
    }
