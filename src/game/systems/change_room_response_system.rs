    ...
    fn run(&mut self, data: Self::SystemData) {
        ...
        while let Some(event) = change_room_events.0.pop() {
            ...
            if start_registry.0.contains(&client_id) {
                let mover = *id_entity_mapping
                    .0
                    .get(&client_id)
                    .expect("Bug: Player id not associated with an entity.");
                let current_room = location_storage
                    .get(mover)
                    .expect("Bug: Player is not in placement.")
                    .0;

                let connected_rooms = connected_locations_storage
                    .get(current_room)
                    .expect("Bug: Room has no connections component.");

                let target_location = {
                    let mut result = None;
                    for location in connected_rooms.0.iter() {
                        let num = number_storage
                            .get(*location)
                            .expect("Bug: Location missing number component.");
                ...
                if let Some(target) = target_location {
                    let move_task = MoveTask { mover, target };
                    move_tasks.0.push(move_task);
                } else {
                    let error = Error::no_target(String::from("You cannot go there.."))
                        .expect("Bug: Invalid error packet.");
                    enqueue_write(write.clone(), LurkMessage::Error(error), client_id);
                }
            } else {
                let error = Error::not_ready(String::from("You have not started."))
                    .expect("Bug: Invalid error packet.");
                enqueue_write(write.clone(), LurkMessage::Error(error), client_id);
            }
        }
